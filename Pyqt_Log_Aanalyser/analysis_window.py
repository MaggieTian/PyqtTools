...
class  AnalysisWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.grid = QGridLayout()  # 设置表格布局
        self.grid.setSpacing(5)   # 设置组件间的间隔
        ...
        for i in range(0,self.cnt):
            ...
            self.grid.addWidget(find_label,i+1,0)
            for j in range(0,2):
                find_txt= QLineEdit()
                self.elemen_dic[str(i+1)+'_'+str(j)] = find_txt  # 以"行_列"的方式存储组件
                self.grid.addWidget(find_txt,i+1,j+1)
            ...
            self.grid.addWidget(result_btn,i+1,j+2)
        ...
        add_btn = QPushButton("清空")
        add_btn.clicked.connect(self.clean_text)  # 添加查找目标和查看结果系列组件
        self.grid.addWidget(add_btn,i+2,j-1)
        self.grid.addWidget(start_btn,i+2,j+2)
    ...
    def analysis(self):
        ...
        if self.elemen_dic:
            ...
            if not self.data:
                ...
                result,self.result_data = self.util.start_analysis(self.log_path,self.data)  # 开始进行查找分析
                if result:
